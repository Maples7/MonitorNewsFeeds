...
class WordTrigger(Trigger):
    ...
    def isWordIn(self, text):
        """
        It returns True if the whole word is present in text, False otherwise
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    if triggerType == "SUBJECT":
        trigger = SubjectTrigger(params[0])  # init by Word
    elif triggerType == "TITLE":
        trigger = TitleTrigger(params[0])
    elif triggerType == "SUMMARY":
        trigger = SummaryTrigger(params[0])
    elif triggerType == "NOT":
        trigger = NotTrigger(triggerMap[params[0]]) # init by Trigger
